...
function filterDoctors(doctors, filter) {
  return doctors.filter((doc) => {
    if (filter.location && !doc.location.toLowerCase().includes(filter.location.toLowerCase()))
      return false;
    if (filter.speciality && !doc.speciality.toLowerCase().includes(filter.speciality.toLowerCase()))
      return false;
    return true;
  });
}
